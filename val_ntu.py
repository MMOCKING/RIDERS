...
def validate(
    ...
    for idx, inputs in enumerate(val_dataloader):
        ...
        if save_output:
            ...
            os.makedirs(os.path.join(output_path, 'SML_any'), exist_ok=True)
            os.makedirs(os.path.join(output_path, 'SML_any', scene), exist_ok=True)
            os.makedirs(os.path.join(output_path, 'SML_any', scene, 'sml_depth'), exist_ok=True)
            os.makedirs(os.path.join(output_path, 'SML_any', scene, 'sml_depth_color'), exist_ok=True)

            # print('Saving output {}'.format(basename))
            data_utils.save_depth(sml_pred, os.path.join(output_path, 'SML_any', scene, 'sml_depth', basename))
            data_utils.save_color_depth(sml_pred, os.path.join(output_path, 'SML_any', scene, 'sml_depth_color', basename), max_depth=max_save_depth)

    time_end = time.time()
    print('Time taken: {:.4f} seconds'.format(time_end - time_start))
    print('average time per sample: {:.4f} seconds'.format((time_end - time_start) / len(val_image_paths)))
...
if __name__ == "__main__":
    ...
    result_root = '/media/lh/lh1/dataset/NTU/output'
    scenes = ['loop2_2022-06-03_1', 'loop3_2022-06-03_0']
    # scenes = ['garden_2022-05-13_0', 'garden_2022-05-13_1']

    checkpoint_dirpath = '/media/lh/lh1/dataset/NTU/log/SML/best-any'
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    min_pred = 0.1
    max_pred = 255.0
