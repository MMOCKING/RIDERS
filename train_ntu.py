...
if __name__ == '__main__':
    ...
    train(
        # data input
        train_root = '/media/lh/lh1/dataset/NTU',
        ...
        result_root = '/media/lh/lh1/dataset/NTU/output', # rcnet output

        # training
        learning_rates = [5e-5,2e-5], #[1e-4,5e-5],[5e-5,5e-5,5e-5]
        learning_schedule = [10,80], #[40,80],
        batch_size = 12,
        ...
        loss_func = 'l1',
        # weights
        w_lidar_loss = 1.0, # the weight of sparse lidar GT depth are heavier than interpolated lidar GT depth
        ...
        max_depth = 100.0,
        checkpoint_dirpath = os.path.join('/media/lh/lh1/dataset/NTU/log/SML', current_time),
        n_threads=4,

        # pipeline
        model_type='midas-small',  # scale map learner network type：midas-small, dpt-large (need large GPU memory)
        interp='rcnet_0.4',  # rcnet: use quasi-dense depth from rcnet，none: use sparse radar points
