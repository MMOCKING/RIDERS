...
def save_depth(z, path, multiplier=256.0):
    ...
    depth = np.uint32(z * multiplier)
    depth = Image.fromarray(depth, mode='I')
    depth.save(path)


def save_color_depth(z, path, max_depth=None):
    '''
    ...
    if max_depth is None:
        ...
        z_normalized = z.copy()
        z_normalized[z_normalized > max_depth] = max_depth
        z_normalized = z_normalized / max_depth
...
def interpolate_depth_UTV(depth_map, validity_map=None, log_space=False, window_size=12):
    '''
    Interpolate sparse depth with barycentric coordinates
    Args:
    depth_map : np.float32
...
def interpolate_depth_delft(depth_map, validity_map=None, log_space=False):
    '''
    Interpolate sparse depth with barycentric coordinates
    Args:
    depth_map : np.float32
    ...
    if validity_map is None:
        validity_map = depth_map > 0.0
        # np.where(depth_map > 0.0, 1.0, 0.0)
    rows, cols = depth_map.shape
    data_row_idx, data_col_idx = np.where(validity_map)
    depth_values = depth_map[data_row_idx, data_col_idx]
    # Perform linear interpolation in log space
