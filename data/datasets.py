...
from data.UTV_dataset import load_sparse_depth
...
class RCNetTrainingDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.pad_size_y = patch_size[0] // 2
        # self.padding = ((0, 0), (0, 0), (self.pad_size_x, self.pad_size_x))
        self.padding = ((0, 0), (self.pad_size_y, self.pad_size_y), (self.pad_size_x, self.pad_size_x))
    ...
    def __getitem__(self, index):
        ...
        # pad image to account for the crop
        image = np.pad(
            image,
        ...
        if self.radar_paths[index].endswith('.npy'):
            ...
            depth_map = load_sparse_depth(self.radar_paths[index])
            # H*W*Depth to n*3 (u,v,depth), depth_map[v, u] = depth
            uvs = np.where(depth_map > 0)
            depth_values = depth_map[depth_map > 0]
            radar_points = np.column_stack((uvs[1], uvs[0], depth_values))
        ...
        # 有0.1的概率，radar点云是由lidar生成的伪点云，随机采样lidar点云，然后加入噪声，制造伪radar点云
        if random.random() < self.sample_probability_of_lidar:
            ...
            noise_for_fake_radar_z = np.random.uniform(low=0.0, high=0.5, size=radar_points.shape[0])
        ...
        for radar_point_idx in range(0, radar_points.shape[0]):
            ...
            radar_points[radar_point_idx, 1] = radar_points[radar_point_idx, 1] + self.pad_size_y

            # Set radar point to the center of the patch
            bounding_box = [0, 0, 0, 0]
            bounding_box[0] = radar_points[radar_point_idx, 0] - self.pad_size_x
            bounding_box[1] = radar_points[radar_point_idx, 1] - self.pad_size_y
            bounding_box[2] = radar_points[radar_point_idx, 0] + self.pad_size_x
            bounding_box[3] = radar_points[radar_point_idx, 1] + self.pad_size_y
        ...
        for radar_point_idx in range(0, radar_points.shape[0]):
            ...
            start_y = int(radar_points[radar_point_idx, 1] - self.pad_size_y)
            end_y = int(radar_points[radar_point_idx, 1] + self.pad_size_y)

            # ground_truth_cropped = ground_truth[:, start_y:, start_x:end_x]
            ground_truth_cropped = ground_truth[:, start_y:end_y, start_x:end_x]
